...
| `flutter_pop` | `timeout_ms: Option<u64>` | Ejecuta el gesto de retroceso estándar de Flutter Driver (`PageBack`). **Limitación validada:** su matcher del lado del SDK solo reconoce un `Tooltip(message: "Back")` literal sin localizar o un `CupertinoNavigationBarBackButton` -- sin fallback genérico por tipo de widget. Confirmado contra una app real en español que **no** matchea ni siquiera el `BackButton` default de Material (su tooltip se localiza y deja de ser "Back"), y mucho menos un botón de retroceso custom (`IconButton`). Solo útil en apps en inglés o Cupertino; en cualquier otro caso, preferí `flutter_tap(by: "key", ...)` sobre el botón de retroceso si tiene una key. |
